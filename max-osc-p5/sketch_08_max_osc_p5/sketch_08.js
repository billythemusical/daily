...
function setup() {
  ...
  for (i = 0; i < num; i++) {
    var x = (i + 1)/num * (width*0.8);
    var y = (i + 1)/num * (height*0.8);
    var w = 80 + (i * 5);
    var h = 80 + (i * 5);
    circles[i] = new Circle(x, y, w, h, 100, 2, filters[i]);
...
function draw() {
  var r = 100 * circles[1].amp * 10;
  var g = 100;
  var b = 50 + (circles[2].amp * 1000);
  // background(r, g, b, 30);
  background(0, 30);
  boundingRect();
  ...
  for(i = 0; i < num; i++) {
    ...
    friction.mult(-1);
    friction.setMag(c);
    let gravity = createVector(0.89, 0.89);
    // friction.add(circles[i].move);
